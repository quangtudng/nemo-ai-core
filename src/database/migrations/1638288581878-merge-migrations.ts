...
export class mergeMigrations1638288581878 implements MigrationInterface {
    name = 'mergeMigrations1638288581878'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`nemo_dev\`.\`users\` (\`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`id\` int NOT NULL AUTO_INCREMENT, \`email\` varchar(100) NOT NULL, \`password\` varchar(100) NOT NULL, \`fullname\` varchar(50) NULL, \`phone_number\` varchar(50) NULL, \`status\` tinyint NOT NULL DEFAULT '0', \`bio\` text NULL, \`role_id\` int NULL, UNIQUE INDEX \`IDX_97672ac88f789774dd47f7c8be\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`nemo_dev\`.\`locations\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(100) NOT NULL, \`type\` varchar(100) NOT NULL, \`slug\` varchar(100) NOT NULL, \`description\` text NULL, \`mpath\` varchar(255) NULL DEFAULT '', \`parent_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);

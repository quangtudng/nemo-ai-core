...
export class initRelationalDb1635954883552 implements MigrationInterface {
    name = 'initRelationalDb1635954883552'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`nemo_dev\`.\`roles\` (\`id\` int NOT NULL AUTO_INCREMENT, \`num\` smallint NOT NULL, \`label\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`nemo_dev\`.\`users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`email\` varchar(100) NOT NULL, \`password\` varchar(100) NOT NULL, \`firstname\` varchar(50) NOT NULL, \`lastname\` varchar(50) NOT NULL, \`status\` tinyint NOT NULL DEFAULT '0', \`role_id\` int NULL, UNIQUE INDEX \`IDX_97672ac88f789774dd47f7c8be\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
